// Seleciona o elemento HTML com a classe "playerdavez" para mostrar o jogador atual
const playerdavez = document.querySelector(".playerdavez");
...
function iniciarJogo() {
  ...
  playerdavez.innerHTML = `VEZ DO JOGADOR: ${player}`;
...
function novaJogada(e) {
  ...
  // Alterna para o próximo jogador (X -> O ou O -> X) e atualiza o elemento "playerdavez"
  player = player === "X" ? "O" : "X";
  playerdavez.innerHTML = `VEZ DO JOGADOR: ${player}`;
}
let pontosx=0
  ...
  for (posicao of positions) {
    if (posicao.every((item) => posicoesJogadas.includes(item))) {
      ...
      if(jogadorUltimaJogada =="X"){
        ...
        pontosx++
        document.getElementById("pontos").textContent=pontosx
      }
      else{
        
        pontos++
        document.getElementById("pontosO").textContent=pontos
